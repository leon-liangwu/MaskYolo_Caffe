...
int main(int argc, char** argv) {
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
	...
	std::string img_path = lines[line_id];
	std::string lab_path;
	int pos = img_path.find_last_of(".");
	lab_path = img_path.replace(pos+1, 3, "txt");
	pos = lab_path.find("JPEGImages",0);
	lab_path.replace(pos, 10, "labels");
	img_path = lines[line_id];
//	std::cout<<img_path<<std::endl;
//	std::cout<<lab_path<<std::endl;

    status = ReadDecDataToDatum(img_path, lab_path,

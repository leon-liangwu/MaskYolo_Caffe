...
def array_to_datum2(arr1, arr2, label=None):
    """Converts a 3-dimensional array to datum. If the array has dtype uint8,
    the output data will be encoded as a string. Otherwise, the output data
    will be stored in float format. 
    """
    if arr1.ndim != 3:
        raise ValueError('Incorrect array shape.')
    datum = caffe_pb2.Datum()
    datum.channels, datum.height, datum.width = arr1.shape
    datum.data = arr1.tostring()
    datum.float_data.extend(arr2.astype(float).flat)
    if label is not None:
        datum.label = label
    return datum

def array_to_datum3(arr1, arr2, shape, label=None):
    """Converts a 3-dimensional array to datum. If the array has dtype uint8,
    the output data will be encoded as a string. Otherwise, the output data
    will be stored in float format.
    """
    datum = caffe_pb2.Datum()
    datum.encoded = False
    datum.height, datum.width, datum.channels = shape
    datum.data = arr1.tostring()
    datum.float_data.extend(arr2.astype(float).flat)
    if label is not None:
        datum.label = label

...
template <typename Dtype>
void get_roimask_gt(Dtype* mask_roi, int target_size, const Dtype* mask_lab, int mask_w, int mask_h, 
    box& roi_box, int instance_id, int box_cls)
{
  float roi_x = (roi_box.x - roi_box.w / 2) * mask_w;
  float roi_y = (roi_box.y - roi_box.h / 2) * mask_h;
  float roi_w = roi_box.w * mask_w;
  ...
  for( int r = 0; r< rect.height; r++)
  {
    ...
    {
      if((int)mask_lab[(r+rect.y) * mask_w + c + rect.x] == instance_id * 20)
...
void rand_sort(vector< vector<float> > &pos_boxes)
{
  int boxes_num = pos_boxes.size();
  for(int i=0; i<boxes_num; i++)
  {
    int swap_ind = rand() % boxes_num;
    if(swap_ind == i ) continue;
    else
    {
      vector<float> box = pos_boxes[i];
      pos_boxes[i] = pos_boxes[swap_ind];
      pos_boxes[swap_ind] = box;
    }
  }
}
...
void DecodeRoisLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  {
    cout<<"box: ";
    for(int j=0; j<pos_boxes[i].size();j++)
    {
      cout<<pos_boxes[i][j]<<" ";
    }
    cout<<endl;
  ...
  */
  ...
  /*
  LOG(INFO)<<"======================";
  LOG(INFO)<<"neg size "<<neg_boxes.size();
  for( int i=0; i<prop_num_; i++)
  {
    LOG(INFO)<<i<<" "<<top_data_0[i*5 + 0]<<" "<<top_data_0[i*5 + 1]<<" "<<top_data_0[i*5 + 2]<<" "<<top_data_0[i*5 + 3]<<" "<<top_data_0[i*5 + 4]<<" "<<top_data_1[i];
  }
  */
  ...
  */
  ...
  for(int b = 0; b<batch_; b++)
  {
    ...
    for(int t=0; t<prop_num_; t++)
    {
      ...
      float x1 = top_data_0[t*5+1] / nw_;
      float y1 = top_data_0[t*5+2] / nh_;
      float x2 = top_data_0[t*5+3] / nw_;
      float y2 = top_data_0[t*5+4] / nh_;
    ...
    draw_boxes(img, gt_boxes_v, Scalar(255,255,255));
    draw_boxes(img, pd_boxes_neg, Scalar(255,0,0));
    draw_boxes(img, pd_boxes_pos, Scalar(0,255,0));
    imshow("img", img);
